...
4. In your GitHub repository settings, under **Secrets and variables → Actions → Variables**, create a variable named `DELAY_BEFORE_DESTRUCTION` with the delay (in seconds) you want.
5. Still under **Secrets and variables → Actions → Secrets**, create a secret named `AZURE_RESOURCE_GROUP` that contains the resource group of the Azure Function. The deployment workflow relies on these values to keep the Azure App Settings in sync.
